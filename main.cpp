 ...
 *
 *        Version:  1.0
 *        Created:  08/19/2015 09:08:03 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
void initlog(const std::string& level)
{
	...
	logging::add_file_log(
        	logging::keywords::format = "%TimeStamp% [%ThreadID%] <%Severity%>: "
					"%Message%",
...
int main(int argc, char **argv)
{
	...
	if (test_config->max_padi_cnt_) {
		std::cout << "Max count of PADI requests: " << test_config->max_padi_cnt_ << std::endl;
	}
	...
	if (!worker->bind_interface(test_config->interface_)) {
		std::cerr << "Fail to bind interface " << test_config->interface_ << std::endl;
		return -1;
	}
