...
public:
	...
	void
	add_unit(unsigned int sid, unsigned int ip, const MacAddr& srv)
	{
		ios_.post(std::bind(&NetWorker::claim_unit, shared_from_this(), sid, ip, (const char*)srv));
...
private:
	...
	task_fill()
	{
		...
		for (; iter != units_.end(); ++iter)
		{
			...
			{
				do {
					hdr = (struct tpacket_hdr*)(((char*)pkt_hdr_start_) + (packet_req_.tp_block_size*cur_use_block_));
					...
					case TP_STATUS_WRONG_FORMAT:
						...
						exit(EXIT_FAILURE);
					default:
						PPP_LOG(trace) << "NetWorker task_fill return hdr->status:" << (uint32_t)(hdr->tp_status);
						usleep(0);
						break;
					}
				} while (loop == 1);

				cur_use_block_++; loop = 1;
	...
	fill_dealer()
	{
		...
		if (!config->max_padi_cnt_ || stats->padi_send_ok_ < config->max_padi_cnt_) {
			timer_.expires_from_now(boost::posix_time::seconds(1));
			timer_.async_wait(boost::bind(&NetWorker::task_fill, shared_from_this()));
		} else {
