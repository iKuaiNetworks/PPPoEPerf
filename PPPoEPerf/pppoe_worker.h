...
public:
	...
	bool bind_interface(const std::string& ifname)
	{
		...
		if (setsockopt(raw_sock_, SOL_SOCKET, SO_BINDTODEVICE, &eth, sizeof(eth)) == -1) {
			return false;
		}
	...
	bool send_frame(const void *buf,
			unsigned int size,
			struct sockaddr* dst,
...
struct PPPSesEvent {
	PPPSesEvent(unsigned short sid, unsigned int timeout): session_id_(sid), timeout_secs_(timeout)
	{}
...
class PPPoEWorker :
	public std::enable_shared_from_this<PPPoEWorker> {
public:
	struct update_sig {};
	struct cancel_sig {};
	struct stop_sig   {};
...
public:
	typedef signals::signal<void (unsigned int sid, unsigned int ip, const MacAddr& mac)> update_signal;
	...
	template <typename S>
	signals::connection
	connect(update_sig s, const S& slot)
	{  return update_.connect(slot); }

	template <typename S>
	signals::connection
	connect(cancel_sig s, const S& slot)
	{  return cancel_.connect(slot); }

	template <typename S>
	signals::connection
	connect(stop_sig s, const S& slot)
...
private:
	...
	bool send_lcp_config_ack(const MacAddr server_mac, unsigned short session_id, unsigned char id, LCPOptList &opt_list);
	bool send_lcp_echo_reply(const MacAddr server_mac, unsigned short session_id, unsigned char id);
	bool send_chap_reply(const MacAddr server_mac, unsigned short session_id, unsigned short id, unsigned char* chal, size_t chal_size);
	...
	bool parsed_pppoe_tag_data(unsigned char *data, unsigned data_size, PPPoEServer &server);
	bool process_pppoe_session_pkt(unsigned char * data, unsigned int size);
	bool parsed_lcp_opt_data(unsigned char *data, unsigned data_size, LCPOptList &opt_list);
	...
	bool get_chap_challenge(const unsigned char* data, unsigned int sz, unsigned char** chal, unsigned int* chal_size);

	bool set_ipcp(unsigned short sid, LCPOptList& opt_list);
	bool send_ipcp_ack(const MacAddr server_mac, unsigned short session_id, unsigned char id);
	bool send_ipcp_req(const MacAddr server_mac, unsigned short session_id, unsigned char id);
	bool send_ipcp(const MacAddr server_mac, unsigned short session_id, unsigned char id, bool req);
	void loop();

	void do_add(unsigned int sid, unsigned int ip, const MacAddr& mac)
	{  update_(sid, ip, mac);  }

	void do_cancel(unsigned int sid)
	{  cancel_(sid); }
...
private:
	...
	std::multimap<uint64_t, uint16_t> onlines_; //time->uid  record the online sid
	std::multimap<uint64_t, std::string> offlines_; //time->name record the offline name
	std::vector<std::string> rlogin_names_;
	std::map<uint16_t, std::string> cache_; //sid->name     record the sid mapped name
