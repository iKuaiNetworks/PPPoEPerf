...
NetUnit::NetUnit(unsigned int id, unsigned int ip,
			const char* srv,
...
int
NetUnit::compose_pppoe(unsigned char* p, unsigned int len)
{
	pkt_ppoe_t pkt;
	struct sockaddr_ll dst;
...
NetWorker::initial()
{
	...
	pkt_hdr_start_ = (struct tpacket_hdr*)mmap(0, size, PROT_READ|PROT_WRITE, MAP_SHARED, fd_sock_, 0);
