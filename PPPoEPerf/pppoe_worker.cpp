...
bool PPPoEWorker::send_padr(struct PPPoEServer &server) {
  ...
  *reinterpret_cast<unsigned short *>(tag_data) = htons(PPPOE_TAG_SERVICE_NAME);
  tag_data += 2;
  *reinterpret_cast<unsigned short *>(tag_data) =
      htons(server.svc_name_.size());
  tag_data += 2;
...
bool PPPoEWorker::send_lcp_config_req(const MacAddr server_mac,
  ...
  pkt.ps.length_ = htons(data_size + sizeof(pkt.ph) +
                         sizeof(pkt.lp));  // the rest payload size
  ...
  *opt = 6;
  ++opt;
  *reinterpret_cast<unsigned int *>(opt) =
      htons(random_ptr_->generate_random_int());
  opt += 4;
...
bool PPPoEWorker::send_lcp_echo_reply(const MacAddr server_mac,
                                      unsigned short session_id,
                                      unsigned char id) {
  union {
    struct {
...
void PPPoEWorker::reset_expired_event(unsigned short sid,
                                      unsigned int timeout) {
  unsigned int expired_time = time(NULL) + timeout;
  PPPTimeoutEvent timeout_event(sid, expired_time);
  PPPSesEvent ses_event(sid, expired_time);
...
void PPPoEWorker::remove_expired_event(unsigned short sid) {
  ...
  if (sit != ppp_ses_event_.end()) {
    ...
    if (eit != ppp_timeout_event_.end()) {
      ...
      PPP_LOG(error) << "PPPoEWorker not find expired event "
                     << sit->timeout_secs_;
    }
...
void PPPoEWorker::process_expired_events(time_t cur_time) {
  ...
  std::multiset<PPPTimeoutEvent>::const_iterator eit =
      ppp_timeout_event_.begin();
  ...
  for (; eit != ppp_timeout_event_.end();) {
    ...
    PPP_LOG(trace) << "PPPoEWorker deal timout list with session "
                   << pe.session_id_;
    ppp_timeout_event_.erase(eit++);
    ...
    if (pit != ppp_entry_.end()) {
      switch (pit->status_) {
        case PPP_STATUS_CONFIG_REQUEST: {
          PPP_LOG(info) << "PPPoEWorker resend LCP config request session id "
                        << pit->session_id_;
...
void PPPoEWorker::recv_pkt(void) {
  ...
  if (-1 == ret) {
    PPP_LOG(error) << "PPPoEWorker failed to receive with error info "
                   << strerror(errno);
    return;
  }
...
bool PPPoEWorker::parsed_pppoe_tag_data(unsigned char *data, unsigned data_size,
                                        PPPoEServer &server) {
...
bool PPPoEWorker::process_pppoe_disc_pkt(unsigned char *data,
                                         unsigned int size) {
  ...
  if (pkt->ver_ != PPPOE_DISC_VER) {
    PPP_LOG(error) << "PPPoEWorker invalid version :"
                   << static_cast<uint8_t>(pkt->ver_);
    stats->invalid_disc_pkt_++;
    return false;
  }
  if (pkt->type_ != PPPOE_DISC_TYPE) {
    PPP_LOG(error) << "PPPoEWorker invalid type:"
                   << static_cast<uint8_t>(pkt->type_);
    stats->invalid_disc_pkt_++;
    return false;
  }
...
*/
    ...
    case PPPOE_DISC_CODE_PADS:
      ...
      if (servers_.find(server) == servers_.end()) {
        PPP_LOG(error)
            << "PPPoEWorker receive a pads responses should not receive";
        return false;
      }

      pkt->session_id_ = ntohs(pkt->session_id_);
      pkt->length_ = ntohs(pkt->length_);
      PPP_LOG(info) << "PPPoEWorker receive session id "
                    << static_cast<uint16_t>(pkt->session_id_);
      if (pkt->length_ > tag_size) {
        PPP_LOG(error) << "PPPoEWorker receive invalid length "
                       << static_cast<uint16_t>(pkt->length_);
        stats->invalid_pads_++;
        return false;
      }
...
*/
      ...
      if (test_config->terminate_) {
        if (send_padt(server.svc_mac_, pkt->session_id_)) {
          PPP_LOG(info) << "PPPoEWorker send padt with sid "
                        << static_cast<uint16_t>(pkt->session_id_);
          stats->padt_send_ok_++;
        } else {
...
bool PPPoEWorker::parsed_lcp_opt_data(unsigned char *data,
...
void PPPoEWorker::set_ppp_auth_method(unsigned short sid,
                                      LCPOptList &opt_list) {
  PPPEntry pe(sid);
...
bool PPPoEWorker::get_chap_challenge(const unsigned char *data, unsigned int sz,
                                     unsigned char **chal,
...
bool PPPoEWorker::send_ipcp_ack(const MacAddr server_mac,
                                unsigned short session_id, unsigned char id) {
  return send_ipcp(server_mac, session_id, id, false);
}

bool PPPoEWorker::send_ipcp_req(const MacAddr server_mac,
                                unsigned short session_id, unsigned char id) {
  return send_ipcp(server_mac, session_id, id, true);
}

bool PPPoEWorker::send_ipcp(const MacAddr server_mac, unsigned short session_id,
  ...
  *ptr = IPCP_IP_ADDR;
  ...
  return sock_ptr_->send_frame(
      &pkt, pkt.data + payload - reinterpret_cast<unsigned char *>(&pkt),
      (struct sockaddr *)&dst, sizeof(dst));
}
...
void PPPoEWorker::deal_ipcp_ack(unsigned int sid) {
  ...
  onlines_.insert(
      std::make_pair((uint64_t)now, sid));  // update the onlines info
  unsigned int ip;
...
void PPPoEWorker::do_relogin() { send_padi(); }
...
bool PPPoEWorker::process_pppoe_session_pkt(unsigned char *data,
                                            unsigned int size) {
  struct eth_frame *frame = reinterpret_cast<struct eth_frame *>(data);
  struct pppoe_pkt *pkt = reinterpret_cast<struct pppoe_pkt *>(frame + 1);
    ...
    case PPP_PROTO_LCP:
      ...
      switch (lp->code_) {
        case LCP_CODE_CONFIG_REQ:
          ...
          if (!parsed_lcp_opt_data(opt_data, lp->len_ - sizeof(*lp),
                                   opt_list)) {
          ...
          if (send_lcp_config_ack(frame->src_, pkt->session_id_, lp->id_,
                                  opt_list)) {
        ...
        case LCP_CODE_CONFIG_ACK:
          ...
          if (!is_valid_session(pkt->session_id_, PPP_STATUS_CONFIG_REQUEST)) {
            PPP_LOG(error)
                << "PPPoEWorker receive lcp config ack with invalid sid "
                << static_cast<uint16_t>(pkt->session_id_);
        ...
        case LCP_CODE_ECHO_REQ:
          ...
          PPP_LOG(info) << "PPPoEWorker receive lcp echo with sid "
                        << static_cast<uint16_t>(pkt->session_id_);

          opt_data = reinterpret_cast<unsigned char *>(lp + 1);
          if (send_lcp_echo_reply(frame->src_, pkt->session_id_, lp->id_)) {
            PPP_LOG(info) << "PPPoEWorker send lcp echo reply with sid "
                          << static_cast<uint16_t>(pkt->session_id_);
        ...
        case LCP_CODE_TERM_REQ:
          PPP_LOG(error) << "PPPoEWorker "
                         << static_cast<uint16_t>(pkt->session_id_)
                         << " receive lcp term reqest";
          do_cancel(pkt->session_id_);
          break;
        case LCP_CODE_TERM_ACK:
          PPP_LOG(error) << "PPPoEWorker "
                         << static_cast<uint16_t>(pkt->session_id_)
                         << " receive lcp term ack";
          do_cancel(pkt->session_id_);
          break;
        case LCP_CODE_PROTO_REJ:
          PPP_LOG(error) << "PPPoEWorker "
                         << static_cast<uint16_t>(pkt->session_id_)
                         << " receive lcp proto rej";
          break;
        case LCP_CODE_DISA_REQ:
          PPP_LOG(error) << "PPPoEWorker "
                         << static_cast<uint16_t>(pkt->session_id_)
                         << " receive lcp disa req";
          break;
        default:
          break;
      }
    ...
    case PPP_PROTO_CHAN_HANDSHAKE_AUTH:
      ...
      switch (lp->code_) {
        case CHAP_CHALLENGE:
          ...
          PPP_LOG(info) << "PPPoEWorker receive chap handshake req with sid "
                        << static_cast<uint16_t>(pkt->session_id_);
          opt_data = reinterpret_cast<unsigned char *>(lp + 1);
          if (get_chap_challenge(opt_data, lp->len_ - sizeof(*lp), &chal_num,
                                 &chal_size)) {
            stats->valid_ppp_chap_challenge_++;
            if (send_chap_reply(frame->src_, pkt->session_id_, lp->id_,
                                chal_num, chal_size)) {
              stats->chap_response_send_ok_++;
            } else {
              stats->chap_response_send_fail_++;
              PPP_LOG(error) << "PPPoEWorker send chap handshake resp with sid "
                             << static_cast<uint16_t>(pkt->session_id_);
            }
            delete chal_num;
          } else {
          ...
          break;
        case CHAP_SUCESS:
          PPP_LOG(info) << "PPPoEWorker receive chap sucess with sid "
                        << static_cast<uint16_t>(pkt->session_id_);
          stats->ppp_chap_success_++;
          break;
        case CHAP_FAILURE:
          PPP_LOG(error) << "PPPoEWorker receive chap failed with sid "
                         << static_cast<uint16_t>(pkt->session_id_);
    ...
    case PPP_PROTO_IPCP:
      ...
      switch (lp->code_) {
        case CONFIG_REQUEST:
          ...
          break;
        case CONFIG_ACK:
          PPP_LOG(info) << "PPPoEWorker receive ipcp ack with sid "
                        << static_cast<uint16_t>(pkt->session_id_);
          stats->ppp_ipcp_recv_ack_++;
          deal_ipcp_ack(pkt->session_id_);
          // todo:parse the ip dns, and enter next step, try to keep some net
          // flow.
          break;
        case CONFIG_NAK:
          opt_data = reinterpret_cast<unsigned char *>(lp + 1);
          ...
          PPP_LOG(info) << "PPPoEWorker receive ipcp nak with sid "
                        << static_cast<uint16_t>(pkt->session_id_);
          if (!send_ipcp_req(frame->src_, pkt->session_id_, lp->id_)) {
            PPP_LOG(error) << "PPPoEWorker send ipcp req failed";
            return false;
          ...
          break;
        case CONFIG_REJECT:
          PPP_LOG(error) << "PPPoEWorker receive ipcp reject with sid "
                         << static_cast<uint16_t>(pkt->session_id_);
          stats->ppp_ipcp_recv_reject_++;
          break;
        case TERMINATE_REQUEST:
          PPP_LOG(error) << "PPPoEWorker receive ipcp terminate req with sid "
                         << static_cast<uint16_t>(pkt->session_id_);
          stats->ppp_ipcp_recv_term_req_++;
          break;
        case TERMINATE_ACK:
          PPP_LOG(error) << "PPPoEWorker receive ipcp terminate ack with sid "
                         << static_cast<uint16_t>(pkt->session_id_);
          break;
        case CODE_REJECT:
          PPP_LOG(error) << "PPPoEWorker receive ipcp code reject with sid "
                         << static_cast<uint16_t>(pkt->session_id_);
